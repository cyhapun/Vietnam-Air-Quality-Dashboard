...
from utils.helpers import AQI_DEF, POLLS, aqi_meta, ml, ax, chart_h, PT, GC, LC, TF, hex_rgba
...
def render(global_df):
    ...
    st.markdown('<div class="card" style="padding: 1.5rem; margin-bottom: 1rem;">', unsafe_allow_html=True)
    ...
    with c3:
        # Use segmented control pattern via radio
    ...
    with c5:
        ...
        polls_lbls = ["AQI (US)"] + [POLLS[k]["label"] for k in POLLS]
        
        idx_pol = 0
        if st.session_state["dt_pollutant"] in polls_keys:
             idx_pol = polls_keys.index(st.session_state["dt_pollutant"])
             
        poll_lbl = st.selectbox("Thông số", polls_lbls, index=idx_pol)
        selected_poll_key = polls_keys[polls_lbls.index(poll_lbl)]
        
        if selected_poll_key != st.session_state["dt_pollutant"]:
            st.session_state["dt_pollutant"] = selected_poll_key
            st.rerun()
    ...
    with cT2:
        lbl_min, c_min = aqi_meta(row_min["aqi"])
        lbl_max, c_max = aqi_meta(row_max["aqi"])
        ...
        <div style="display:flex; justify-content: flex-end; gap: 12px; align-items:center; height:100%;">
            ...
            <div style="background:{hex_rgba(c_min, 0.12)}; border: 1.5px solid {hex_rgba(c_min, 0.4)}; padding: 10px 14px; border-radius: 10px; display:flex; flex-direction:column; min-width:140px;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:4px;">
                    <span style="font-size:22px; font-weight:700; color:{c_min}; line-height:1;">{val_min:.0f if y_col == "aqi" else f"{val_min:.1f}"}</span>
            ...
            <div style="background:{hex_rgba(c_max, 0.12)}; border: 1.5px solid {hex_rgba(c_max, 0.4)}; padding: 10px 14px; border-radius: 10px; display:flex; flex-direction:column; min-width:140px;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:4px;">
                    <span style="font-size:22px; font-weight:700; color:{c_max}; line-height:1;">{val_max:.0f if y_col == "aqi" else f"{val_max:.1f}"}</span>
    ...
    # Prepare array colors & labels for Plotly
    df_sub["clr"] = df_sub["aqi"].apply(lambda x: aqi_meta(x)[1])
    df_sub["lbl"] = df_sub["aqi"].apply(lambda x: aqi_meta(x)[0])
    ...
    for lo, hi, lbl, col in AQI_DEF:
        legend_html += f'''
        <div style="display:flex; align-items:center; gap:6px; padding:4px 10px; border-radius:99px; background:{hex_rgba(col,0.1)}; border: 1px solid {hex_rgba(col, 0.4)}">
            <div style="width:10px; height:10px; border-radius:50%; background:{col};"></div>
            <span style="color:{col}; font-weight:600;">{lbl} ({lo}-{hi})</span>
        </div>
        '''
    legend_html += '</div>'
